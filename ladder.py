...
class onecode:
    def __init__(self,host='',user='',passwd='',db='',port=3306,charset='utf8',user_problem='',judge_detail='',problem='',ladder_problem=''):
        self.host = host
        self.user = user
        self.passwd = passwd
        self.db = db
        ...
        self.ladder_problem=ladder_problem
        self.conn = None
        self._conn()
    ...
    def modifyUserMessage(self,row):
        ...
        sql = "select  * from %s where  user_id = '%d' " % (self.user_problem, row['user_id'])
        all_judge_record=self.select(sql)
        score=0
        for each in all_judge_record:
            sql = "select  * from %s where  user_problem_id = '%d' " % ('ladder_judge_detail', each['id'])
            all_detail_judge=self.select(sql)
            tmp_score=0
            for detail in all_detail_judge:
                if detail['judge_status'] == 0:
                    score=score+int(detail['group_score'])
                    tmp_score=tmp_score+int(detail['group_score'])
            sql = "UPDATE ladder_user_problem SET score = '%d' WHERE id = '%d'" % (tmp_score, each['id'])
            self.update(sql)
        sql = "UPDATE user SET ladder_solve_problem = '%d',ladder_submissions='%d',ladder_score='%d' WHERE id = '%d'" % (solve_problem, allsubmissionsNumber, score, row['user_id'])
        self.update(sql)
    ...
    def modifyProblemMessage(self,row,status):
        ...
        allsubmissionsNumber=len(self.select(sql))
        sql = "UPDATE %s SET accepted = '%d',submissions='%d' WHERE problem_id = '%d'" % (self.ladder_problem,acceptedNumber, allsubmissionsNumber, row['id'])
        self.update(sql)
    ...
    def Query(self):
        ...
        for row in results:
            ...
            if self.InputLimitExceeded(row) == True:
                ...
                self.modifyUserMessage(row)
                self.modifyProblemMessage(problemData[0], 7)
                continue
            ...
            if self.Compiling(row) == True:
                ...
                self.modifyUserMessage(row)
                self.modifyProblemMessage(problemData[0], 5)
                continue
...
class myThread(threading.Thread):
    def __init__(self,user_problem,judge_detail,problem,ladder_problem):
    ...
    def run(self):
        judge = onecode('localhost', 'root', 'OneCode', 'onecode', 3306, 'utf8', self.user_problem, self.judge_detail,self.problem,self.ladder_problem)
        judge.polling()

if __name__ == '__main__':
    library = myThread('ladder_user_problem','ladder_judge_detail','problem','ladder_contest_problem')
    library.start()
